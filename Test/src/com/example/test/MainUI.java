 ...
 * 
 */
public class MainUI extends RelativeLayout { //自定义view就是这么写
    ...
    public static final int Left_ID = 0xaabbcc;//十六进制
    ...
    private void initview(Context context) {
        ...
        MiddleMask.setBackgroundColor(0x88000000);// 原来设置颜色还可以这样设置，66666，这里设置的最初的颜色，这里是灰色，但是不是很深
        ...
        MiddleMask.setAlpha(0);// 最开始肯定是透明的
    }

    /**
     * 我们在这个方法中监听事件的变化，并设定透明度
     */

    @Override
    public void scrollTo(int x, int y) {//根据滑动的距离变化而变化，因此要想在滑动的时候有什么动作，就可以放在这个方法中，后面执行这个方法就可以了
        // TODO Auto-generated method stub
        super.scrollTo(x, y);
        int curX=Math.abs(getScrollX());//根据这个变化而变化
        float scale=curX/(float)LeftMenu.getMeasuredWidth();//设置比例
    ...
    /**
     * 下面就写一个监听事件来判断是向左还是向右滑动，自己写事件分发，后面理解的事件分发的意思应该也就是不同的事件执行不同的逻辑
     * 不过本实战是通过两个if语句，先对事件进行了判断，然后根据事件作出相应的动作 
     * 
     */
    @Override
